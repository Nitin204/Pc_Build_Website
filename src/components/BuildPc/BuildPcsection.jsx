...
const BuildPcsection = () => {
    ...
    useEffect(() => {
        axios.get(API_BASE)
            .then(res => setPcBuilds(res.data))
            .catch(err => console.error("Fetch Error:", err));
    }, []);

    const componentFields = [
        "Processor", "Chipset", "RAM", "GPU",
        "Cooler", "Storage", "Power Supply", "Cabinet"
    ];

    const subFieldMapping = {
        "Processor": [
            { label: "Platform", key: "platform" },
            { label: "Processor Series", key: "processor_series" },
            { label: "GPU Type", key: "gpu_type" },
            { label: "Processor Model", key: "processor_model" },
        ],
        "Chipset": [{ label: "Chipset Model", key: "chipset_model" }],
        "RAM": [
            { label: "RAM RGB", key: "ram_rgb" },
            { label: "RAM Size", key: "ram_size" },
        ],
        "GPU": [
            { label: "GPU Brand", key: "gpu_brand" },
            { label: "GPU Model", key: "gpu_model" },
        ],
        "Cooler": [{ label: "Cooler Model", key: "cooler_model" }],
        "Storage": [
            { label: "Primary Storage", key: "primary_storage" },
            { label: "Secondary Storage", key: "secondary_storage" },
        ],
        "Power Supply": [{ label: "PSU Model", key: "psu_model" }],
        "Cabinet": [
            { label: "Cabinet Brand", key: "cabinet_brand" },
            { label: "Cabinet Model", key: "cabinet_model" },
        ],
    };
    ...
    // ✅ BUILD MATCH
    const handleBuildPC = () => {
        const match = pcBuilds.find(build =>
            Object.entries(build.specs).every(
                ([key, val]) => formData[key] === val
            )
        );

        if (!match) {
            alert("No matching PC build found!");
            return;
        }
...
const handleAddToCart = async (product) => {
  ...
  window.dispatchEvent(new Event("cartUpdated"));
    ...
    const buildPrice = selectedBuild?.discountPrice || 0;
            ...
            <div className="flex justify-center">
                <div className="bg-gray-950 text-white p-6 md:p-10 w-full max-w-lg rounded-lg border border-gray-800">
                    ...
                    {componentFields.map(field => {
                        ...
                        const selectedValues = subFieldMapping[field]
                            .map(sub => formData[sub.key])
                            .filter(Boolean)
                            .join(', ');
                                ...
                                <div className="p-4 bg-gray-900 space-y-3">
                                    {subFieldMapping[field].map(sub => (
                                        ...
                                        >
                                            ...
                                            {[...new Set(pcBuilds.map(b => b.specs[sub.key]))]
                                                .filter(Boolean)
                                                .map(opt => (
